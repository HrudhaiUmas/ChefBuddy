        ...
        NavigationStack {
            ZStack(alignment: .topTrailing) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    // Recipes View — replaces placeholder
                ...
                HStack {
                    ...
                    Button(action: {
                        ...
                        withAnimation(.spring(response: 0.4, dampingFraction: 0.7)) { showDropdown.toggle() }
                    }) {
                ...
                if showDropdown {
                    Color.black.opacity(0.001).ignoresSafeArea()
                        .onTapGesture { withAnimation(.spring(response: 0.4, dampingFraction: 0.7)) { showDropdown = false } }

                    VStack(alignment: .leading, spacing: 12) {
                        MenuRow(icon: "slider.horizontal.3", title: "Profile / Preferences") {
                            showDropdown = false; showSettings = true
                        }
                        Divider().padding(.horizontal, 16)
                        MenuRow(icon: "arrow.right.square.fill", title: "Log Out", color: .red) {
                            showDropdown = false; authVM.signOut()
                        }
                    }
                    .padding(.vertical, 16)
