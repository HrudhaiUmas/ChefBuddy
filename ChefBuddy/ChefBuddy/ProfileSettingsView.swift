...
class ProfileFormState: ObservableObject {
    ...
    func load(from profile: DBUser?) {
        ...
        dislikesList = Set(profile.dislikes.split(separator: ",").map { $0.trimmingCharacters(in: .whitespaces) }.filter { !$0.isEmpty })
...
struct InitialPreferencesView: View {
        ...
        ZStack {
            ...
            VStack {
                ...
                Button(action: finishSetup) {
                    HStack {
                        if isSaving { ProgressView().tint(.white) } else { Text("Complete Setup").font(.system(size: 18, weight: .bold, design: .rounded)) }
                    }
                ...
                }.padding(.horizontal, 24).padding(.bottom, 20).disabled(isSaving)
            }
        }
    }
    ...
    private func finishSetup() {
        ...
        authVM.saveUserPreferences(
            level: formState.chefLevel, diets: formState.dietTags, allergy: formState.allergies, macros: formState.macroTags,
            age: String(format: "%.0f", formState.age), height: String(format: "%.0f", formState.heightInches), weight: String(format: "%.0f", formState.weight), targetGoal: formState.targetGoal, activity: formState.activityLevel,
            appliances: formState.appliances, cookTime: formState.cookTime, mealPrep: false, cuisines: formState.cuisines,
            spice: formState.spiceTolerance, dislikes: formState.dislikesList.joined(separator: ", "), servings: formState.servingSize, budget: formState.budget
        )
    }
}
...
struct ProfileSettingsView: View {
    ...
    var body: some View {
        ZStack {
            ProfileBackground()
            PreferencesFormContent(formState: formState, title: "Your Kitchen Profile", subtitle: "Tweak your preferences for better AI suggestions.")
            
            VStack {
                Spacer()
                Button(action: saveChanges) {
                    HStack {
                        if isSaving { ProgressView().tint(.white) } else { Text("Save Changes").font(.system(size: 18, weight: .bold, design: .rounded)) }
                    }
                    .foregroundStyle(.white).frame(maxWidth: .infinity).frame(height: 60)
                    .background(LinearGradient(colors: [.orange, .green.opacity(0.9)], startPoint: .leading, endPoint: .trailing))
                    .clipShape(Capsule()).shadow(color: .orange.opacity(0.3), radius: 10, y: 5)
        ...
        .toolbar {
            ToolbarItem(placement: .topBarLeading) {
                Button(action: { dismiss() }) {
                    Image(systemName: "arrow.left.circle.fill").font(.system(size: 28)).foregroundStyle(Color.primary.opacity(0.8), .ultraThinMaterial)
                }
            }
        }
        .onAppear { formState.load(from: authVM.currentUserProfile) }
    }
...
private struct PreferencesFormContent: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    HStack { Spacer(); ProfileSequentialIconView().scaleEffect(0.6).frame(height: 100); Spacer() }
                    VStack(alignment: .leading, spacing: 8) {
                        Text(title).font(.system(size: 34, weight: .heavy, design: .rounded))
                        Text(subtitle).font(.system(size: 17)).foregroundStyle(.secondary).lineSpacing(4)
                ...
                AnimatedSection(isVisible: showSections[0]) {
                    VStack(alignment: .leading, spacing: 28) {
                        ...
                        ProfileSinglePreference(title: "Experience Level", options: levelsList, selected: $formState.chefLevel) { activeSheet = .level }
                        ProfileMultiPreference(title: "Dietary Preferences", options: dietsList, selected: $formState.dietTags) { activeSheet = .diet }
                        ProfileMultiPreference(title: "Avoid (Allergies)", options: allergiesList, selected: $formState.allergies) { activeSheet = .allergy }
                        ProfileMultiPreference(title: "Macro Goals", options: macrosList, selected: $formState.macroTags) { activeSheet = .macro }
                    }
                }
                ...
                AnimatedSection(isVisible: showSections[1]) {
                    VStack(alignment: .leading, spacing: 28) {
                        ...
                        VStack(spacing: 32) {
                            ProfileSlider(title: "Age", value: $formState.age, range: 10...100, format: "%.0f yrs") { activeSheet = .physical }
                            ProfileHeightSlider(title: "Height", inches: $formState.heightInches) { activeSheet = .physical }
                            ProfileSlider(title: "Weight", value: $formState.weight, range: 50...400, format: "%.0f lbs") { activeSheet = .physical }
                        ...
                        ProfileSinglePreference(title: "Target Goal", options: goalsList, selected: $formState.targetGoal) { activeSheet = .goal }
                        ProfileSinglePreference(title: "Activity Level", options: activityList, selected: $formState.activityLevel) { activeSheet = .activity }
                    }
                }
                ...
                AnimatedSection(isVisible: showSections[2]) {
                    VStack(alignment: .leading, spacing: 28) {
                        ...
                        ProfileMultiPreference(title: "Appliances You Own", options: appliancesList, selected: $formState.appliances) { activeSheet = .appliances }
                        ProfileSinglePreference(title: "Typical Cook Time", options: cookTimesList, selected: $formState.cookTime) { activeSheet = .cookTime }
                    }
                }
                ...
                AnimatedSection(isVisible: showSections[3]) {
                    VStack(alignment: .leading, spacing: 28) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ProfileMultiPreference(title: "Favorite Cuisines", options: cuisinesList, selected: $formState.cuisines) { activeSheet = .cuisines }
                            
                            // Dynamically added Cuisine Chips
                                ...
                                ScrollView(.horizontal, showsIndicators: false) {
                                    HStack(spacing: 12) {
                                        ...
                                        ForEach(customCuisines, id: \.self) { item in
                                            Button {
                                                // FIX: explicitly assign to _ to satisfy compiler
                                                withAnimation { _ = formState.cuisines.remove(item) }
                                            } label: {
                                                HStack(spacing: 6) {
                                                    Text(item).font(.system(size: 15, weight: .semibold, design: .rounded))
                                                    Image(systemName: "xmark.circle.fill").font(.system(size: 14)).foregroundStyle(.white.opacity(0.8))
                                                }.foregroundStyle(.white).padding(.horizontal, 16).padding(.vertical, 10).background(Color.green).clipShape(Capsule())
                                            }.buttonStyle(.plain)
                                        }
                                        Spacer().frame(width: 12)
                                    }
                            ...
                            // Text Field to add Cuisines
                            HStack(spacing: 12) {
                                ProfileTextField(placeholder: "Add a custom cuisine...", text: $formState.customCuisineInput)
                                Button(action: {
                                    let input = formState.customCuisineInput.trimmingCharacters(in: .whitespaces)
                                    if !input.isEmpty {
                                        UIImpactFeedbackGenerator(style: .light).impactOccurred()
                                        let newItem = "🍽️ " + input
                                        withAnimation { _ = formState.cuisines.insert(newItem) }
                                        formState.customCuisineInput = ""
                                    }
                                }) {
                                    Image(systemName: "plus.circle.fill").font(.system(size: 38)).foregroundStyle(formState.customCuisineInput.isEmpty ? Color.gray.opacity(0.3) : Color.green)
                                }
                            }.padding(.horizontal, 24)
                        }
                        
                        ProfileSinglePreference(title: "Spice Tolerance", options: spiceList, selected: $formState.spiceTolerance) { activeSheet = .spice }
                        
                        // Ingredients to Avoid Custom Entry
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                Text("Ingredients to Avoid").font(.system(size: 18, weight: .bold, design: .rounded))
                                Button(action: { activeSheet = .dislikes }) { Image(systemName: "info.circle.fill").foregroundStyle(Color.primary.opacity(0.4)) }
                            }.padding(.horizontal, 24)
                            
                            // Dynamically added Dislike Chips
                                ...
                                ScrollView(.horizontal, showsIndicators: false) {
                                    HStack(spacing: 12) {
                                        ...
                                        ForEach(Array(formState.dislikesList).sorted(), id: \.self) { item in
                                            Button {
                                                // FIX: explicitly assign to _
                                                withAnimation { _ = formState.dislikesList.remove(item) }
                                            } label: {
                                                HStack(spacing: 6) {
                                                    Text(item).font(.system(size: 15, weight: .semibold, design: .rounded))
                                                    Image(systemName: "xmark.circle.fill").font(.system(size: 14)).foregroundStyle(.white.opacity(0.8))
                                                }.foregroundStyle(.white).padding(.horizontal, 16).padding(.vertical, 10).background(Color.red).clipShape(Capsule())
                                            }.buttonStyle(.plain)
                                        }
                                        Spacer().frame(width: 12)
                                    }
                            ...
                            // Text Field to add Dislikes
                            HStack(spacing: 12) {
                                ProfileTextField(placeholder: "e.g. Cilantro, Olives...", text: $formState.customDislikeInput)
                                Button(action: {
                                    let input = formState.customDislikeInput.trimmingCharacters(in: .whitespaces)
                                    if !input.isEmpty {
                                        UIImpactFeedbackGenerator(style: .light).impactOccurred()
                                        // FIX: explicitly assign to _
                                        withAnimation { _ = formState.dislikesList.insert(input) }
                                        formState.customDislikeInput = ""
                                    }
                                }) {
                                    Image(systemName: "plus.circle.fill").font(.system(size: 38)).foregroundStyle(formState.customDislikeInput.isEmpty ? Color.gray.opacity(0.3) : Color.red)
                                }
                            }.padding(.horizontal, 24)
                        }
                        
                        ProfileSinglePreference(title: "Typical Serving Size", options: servingsList, selected: $formState.servingSize) { activeSheet = .serving }
                        ProfileSinglePreference(title: "Budget", options: budgetList, selected: $formState.budget) { activeSheet = .budget }
                    }
                }
        ...
        .sheet(item: $activeSheet) { sheetType in SheetRouter(sheetType: sheetType) }
    }
}
...
private struct ProfileBackground: View {
    var body: some View {
        ZStack {
            ...
            Circle().fill(Color.orange.opacity(0.12)).blur(radius: 80).offset(x: -150, y: -300)
            Circle().fill(Color.green.opacity(0.1)).blur(radius: 80).offset(x: 150, y: 300)
        }
    }
}
...
private struct ProfileSectionHeader: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(title).font(.system(size: 14, weight: .bold, design: .rounded)).foregroundStyle(.secondary).textCase(.uppercase)
            VStack { Divider() }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Button(action: onInfoTap) { Image(systemName: "info.circle.fill").foregroundStyle(Color.primary.opacity(0.4)) }
                Spacer()
                Text(String(format: format, value)).font(.system(size: 16, weight: .semibold, design: .monospaced)).foregroundStyle(.orange)
            }
                ...
                HStack(spacing: 12) {
                    ...
                    ForEach(options, id: \.self) { option in
                        ...
                        Button {
                            ...
                            withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) { selected = option }
                        } label: {
                            Text(option).font(.system(size: 15, weight: .semibold, design: .rounded))
                    ...
                    ForEach(options, id: \.self) { option in
                        ...
                        Button {
                            ...
                            withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
                                if isSelected { _ = selected.remove(option) } else { _ = selected.insert(option) }
                            }
                        } label: {
                            Text(option).font(.system(size: 15, weight: .semibold, design: .rounded))
                                .foregroundStyle(isSelected ? .white : .primary)
...
private struct ProfileSequentialIconView: View {
    ...
    var body: some View {
        ZStack {
            Circle().fill(LinearGradient(colors: [.orange.opacity(0.15), .green.opacity(0.15)], startPoint: .topLeading, endPoint: .bottomTrailing)).frame(width: 180, height: 180).scaleEffect(isAnimating ? 1.0 : 0.8)
            Image(systemName: icons[currentIndex]).font(.system(size: 70, weight: .light)).foregroundStyle(LinearGradient(colors: [.orange, .green], startPoint: .topLeading, endPoint: .bottomTrailing)).scaleEffect(isAnimating ? 1.0 : 0.5).opacity(isAnimating ? 1.0 : 0.0).id(currentIndex)
        }
        .onAppear { withAnimation(.spring(response: 0.6, dampingFraction: 0.6)) { isAnimating = true } }
...
private enum ActiveSheet: String, Identifiable {
    case level, diet, allergy, macro, physical, goal, activity, appliances, cookTime, cuisines, spice, dislikes, serving, budget
...
private struct SheetRouter: View {
    ...
    var body: some View {
        ...
        case .physical:
            ProfileSimpleInfoSheet(title: "Body Metrics", message: "We use your Age, Height, and Weight to calculate your Basal Metabolic Rate (BMR) and BMI. This allows the AI to perfectly portion your meals and suggest precise calorie counts!").presentationDetents([.fraction(0.35)])
        ...
        case .cuisines:
            ProfileSimpleInfoSheet(title: "Cuisines", message: "Select your favorite flavors, or add your own custom cuisines using the text box! We'll tailor the recipe styles to match your cravings.").presentationDetents([.fraction(0.35)])
        ...
        case .dislikes:
            ProfileSimpleInfoSheet(title: "Ingredients to Avoid", message: "Not an allergy, just a preference! If you hate cilantro or mushrooms, put them here and you'll never see them in a recipe.").presentationDetents([.fraction(0.35)])
...
// Reusable detailed layout for the info sheets
                    ...
                    ForEach(items, id: \.1) { item in
                        HStack(alignment: .top, spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text(item.2).font(.subheadline).foregroundStyle(.secondary).lineSpacing(2)
                            }
                            Spacer()
                        }
                    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                Text(title).font(.system(size: 24, weight: .bold, design: .rounded))
                Spacer()
                Image(systemName: "info.circle.fill").foregroundStyle(.orange).font(.title2)
            }
            Text(message).font(.system(size: 16, weight: .regular)).foregroundStyle(.secondary).lineSpacing(4)
            Spacer()
        }
        .padding(24).padding(.top, 16)
        .presentationDragIndicator(.visible)
